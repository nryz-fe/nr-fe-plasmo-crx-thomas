...
axios_xyz.interceptors.request.use((config) => {
  // 对请求进行拦截加xyz
...
axios_xyz.interceptors.response.use((response) => {
...
axios_default.interceptors.response.use((response) => {
  ...
  if (response.config.url.includes("proxy.pac")) {
    ...
    resData = {
      success: false,
      message:
        (response.data.base_resp && response.data.base_resp.err_msg) ||
        "出错了，请稍后再试~"
    }
  }
  return resData
